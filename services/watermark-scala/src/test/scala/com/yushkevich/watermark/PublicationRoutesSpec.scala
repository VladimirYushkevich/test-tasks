...
class PublicationRoutesSpec extends WordSpec with Matchers with ScalaFutures with ScalatestRouteTest with JsonSupport {
  ...
  s"GET $base/publications" should {
    "return no publications if no present" in {
      val testActorRef: TestActorRef[PublicationActor] = TestActorRef(new PublicationActor(null) {
      ...
      val testActorRef: TestActorRef[PublicationActor] = TestActorRef(new PublicationActor(null) {
        override def receive: Receive = {
          case GetPublication(_) =>
            val originalSender = sender
            for {
              publication <- Future.successful(Some(Publication("Content", "Author", "Title", Some("watermarkId"), Some("ticketId"))))
            } yield {
  ...
  s"DELETE $base/publications" should {
    ...
    "return INTERNAL SERVER ERROR when exception raised during deletion" in {
      val testActorRef: TestActorRef[PublicationActor] = TestActorRef(new PublicationActor(null) {
        override def receive: Receive = {
          case DeletePublication(_) =>
            sender ! new IllegalStateException("Deletion failed")
        }
      })
