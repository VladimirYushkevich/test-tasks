...
trait WatermarkGenerator {
  def generate(input: Publication): Future[String]
}

object Md5WatermarkGenerator extends WatermarkGenerator with LazyLogging {

  override def generate(input: Publication): Future[String] = Future {
    logger.info(s"Generating watermark for input: $input")

    val md = MessageDigest.getInstance("MD5")
    val digest = md.digest(input.getWatermarkProps.getBytes)
